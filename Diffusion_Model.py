...
class WeightedLoss(nn.Module):
    ...
    def forward(self, pred, targ, weighted=1.0):
        '''
        pred,tag:[batch_size,action_dim]
        '''
...
class SinusoidalPosEmb(nn.Module):  # diffusionQL的编码方式
...
class MLP(nn.Module):
    def __init__(self, state_dim, action_dim, hidden_dim, device, t_dim=16):
        ...
        self.time_mlp = nn.Sequential(
            SinusoidalPosEmb(t_dim),  # 位置编码
            nn.Linear(t_dim, t_dim * 2),  # 线性层
            nn.Mish(),  # 激活函数一般用mish
            nn.Linear(t_dim * 2, t_dim)  # 映射回time维度
        )
...
state_dim:对时间的离散方式：线性、余弦...
clip_denoised:对结果是否clip
kwargs:标识符，不能确定参数是多少可以加上标识符，本质上是字典
'''
...
class Diffusion(nn.Module):
    def __init__(
        ...
        if beta_schedule == "linear":
            betas = torch.linspace(
                0.0001, 0.02, self.T, dtype=torch.float32
            )
        # elif 其他对时间的离散方式
        alphas = 1.0 - betas
        alphas_cumprod = torch.cumprod(alphas, axis=0)  # alphaT一坝[1,2,3] -> [1,2,6]
        ...
        self.loss_fn = Losses[loss_type]()  # loss_fn有两种方式，一范数or二范数
    ...
    def p_mean_variance(self, x, t, s):
        ...
        x_recon.clamp_(-1, 1)  # 下划线进行"in place"操作，不需要进行复制
    ...
    def p_sample_loop(self, state, shape, *args, **kwargs):
        ...
        x = torch.randn(shape, device=device, requires_grad=False)  # 反向过程的梯度
    ...
    def sample(self, state, *args, **kwargs):
        ...
        """
        batch_size = state.shape[0]
        shape = [batch_size, self.action_dim]  # 初始化噪声，规定大小
        action = self.p_sample_loop(state, shape, *args, **kwargs)  # 反向扩散
        return action.clamp_(-1, 1)  # 防止超出边界
